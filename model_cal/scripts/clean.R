...
# Add er variable for calibration
d$er <- d$e.rel
...
# And another dummry variables for any crop x application mthd
...
# Application rate x injection
d$app.rate.os <- d$app.rate * (d$app.mthd == 'os')
d$app.rate.cs <- d$app.rate * (d$app.mthd == 'cs')
# Application rate x *not* injection (ni)
# Note that this predictor is not centered
...
# Re-calculate cumulative rain as average value in interval
