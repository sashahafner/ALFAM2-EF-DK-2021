...
addALFAMVars <- function(d) {
  ...
  d$wind.1m <- d$wind*log(2.0/z0)/log(d$wind.z/z0)
  ###################################################################################################################
  ...
  d <- ddply(d, 'cpmid', transform, ct = cumsum(dt), mt = cumsum(dt) - dt/2, 
	     ##j.rel2 = j.NH3/j.NH3[1], 
             e.cum = cumsum(e.int), ##tan.start = tan.app - cumsum(e.int) + e.int, tan.end = tan.app - cumsum(e.int), 
  ...
  et.app <- as.numeric(difftime(d$t.end, d$app.start, units = 'hours'))
...
summALFAM <- function(d, normalize = FALSE, ...) {
  ...
  ds <- ddply(d, c('database', 'institute', 'inst', 'country', 'file', 'proj', 'exper', 'exper2', 'eid', 'field', 'plot', 'pid', 'pmid'), 
	      ...
	      # Weather, weighted means for most
              ...
              wind.1m.6  = if(any(ct<=6))  sum((wind.1m*dt)[ct<= 6] )/sum(dt[ct<= 6])  else NA,
              wind.1m.12 = if(any(ct<=12)) sum((wind.1m*dt)[ct<= 12])/sum(dt[ct<= 12]) else NA,
              wind.1m.24 = if(any(ct<=24)) sum((wind.1m*dt)[ct<= 24])/sum(dt[ct<= 24]) else NA,
              wind.1m.48 = if(any(ct<=48)) sum((wind.1m*dt)[ct<= 48])/sum(dt[ct<= 48]) else NA,
              wind.1m.mn = sum(wind.1m*dt)/sum(dt),
...
rounddf <- function(x, digits = 2, fun = signif) {
  for(i in 1:ncol(x)) if(class(x[, i])[1] == 'numeric') x[, i] <- fun(x[, i], digits)
  return(x)
}
...
# Now improved, first argument can be empty data.frame
rbindf <- function(...) {
